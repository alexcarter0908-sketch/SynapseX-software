...
describe("engineering intelligence contracts", () => {
  ...
  it("fills in missing arrays instead of discarding an otherwise-usable Builder proposal", () => {
    // Small local models often omit an empty array field or use null instead of [].
    // The parser should salvage everything usable rather than throwing the whole reply away.
    const result = parseBuildProposal({ analysis: "Partial response with only analysis and commands", commands: ["ollama list"] });
    expect(result.analysis).toBe("Partial response with only analysis and commands");
    expect(result.commands).toEqual(["ollama list"]);
    expect(result.plan).toEqual([]);
    expect(result.files).toEqual([]);
  });

  it("still rejects a Builder response with no usable content at all", () => {
    expect(() => parseBuildProposal({})).toThrow("Proposal has no usable content");
  });
  ...
  it("rejects contradictory target and runtime combinations before generation or diagnosis", async () => {
    ...
    const context = { targetId: "windows-powershell" as const, projectType: "Script", runtime: "Bash", permissionLevel: "standard" as const, targetConfirmed: false };
    await expect(caller.builder.generate({ prompt: "Create a backup script for my project", generationMode: "free", context })).rejects.toMatchObject({ code: "BAD_REQUEST", message: expect.stringContaining("PowerShell-compatible") });
    await expect(caller.builder.diagnose({ originalRequirement: "Repair the backup script", terminalEvidence: ["command failed"], context, priorCommands: ["bash ./backup.sh"], priorVerification: ["run the backup check"] })).rejects.toMatchObject({ code: "BAD_REQUEST", message: expect.stringContaining("PowerShell-compatible") });
  });
