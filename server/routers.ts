...
import { buildUniversalGenerationContract, createFreeFirstArtifactPlan, evaluateGenerationEligibility, getExecutionTier, validateTargetRuntimeCompatibility, type UniversalGenerationContext } from "../shared/universalContract";
...
type BuildProposal = {
  ...
  verification: string[];
  commands: string[];
  // Optional, parallel to commands (same index) — a plain-language note per command covering
  // what it does, exactly how/where to run it, and what a successful result looks like. Kept as
  // a separate optional field (rather than changing commands itself) so nothing that already
  // reads commands as plain strings — the safety checks, the ZIP export, existing tests — needs
  // to change.
  commandNotes?: string[];
  risks: string[];
...
async function requestLocalOllamaProposal(prompt: string, contract: string, projectContext?: EngineeringProjectContext): Promise<BuildProposal> {
  ...
  const instruction = `You are SynapseX Local Engineering Assistant. Return ONLY one JSON object with keys: analysis, plan, files, diffs, operations, fileActions, verification, commands, commandNotes, risks (files need path/purpose/content; diffs need path/diff; fileActions need action Create|Update|Delete, path, reason). commandNotes is the same length as commands, in the same order — one plain-language note per command covering what it does, exactly how/where to run it, and what success vs. a likely error looks like.
Rules:
- Simple task (check/change a setting, run a tool) with no new script/project/app requested: files, diffs, operations, fileActions = []; put the real ready-to-run command(s) in commands. commands must not be empty when the user wants something done on the computer.
- Never write a placeholder like <username> or [name] in a command. Use a value already known from the conversation, or a dynamic reference like $env:USERNAME.
- Roman Urdu/Hindi: "karo/kro/kr do/hatao/hata do" = imperative "do it/remove it", never a name or username.
- "Remove/clear a password" (e.g. elevated "net user USERNAME *", blank at both prompts) keeps the account; do NOT use Remove-LocalUser or "/delete" unless the user explicitly asked to delete the account.
- Never claim execution happened. Never fabricate testimonials, reviews, metrics, or security validation. Refuse destructive/bypass/credential-theft/jailbreak requests.
- Reply in the same language/script the user wrote in (analysis, plan, verification, risks); keep code/commands/paths in standard technical form.
- Only produce files/a project if one was actually requested; otherwise keep to commands only.
...
${prompt}`;
  try {
    const response = await fetch(`${baseUrl}/api/generate`, {
      ...
      body: JSON.stringify({ model, prompt: instruction, stream: false, format: "json", options: { temperature: 0.2, num_ctx: 8192 } }),
...
async function requestLocalOllamaChatReply(messages: Array<{ role: "user" | "assistant" | "system"; content: string }>): Promise<string> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 240_000);
  const baseUrl = (process.env.SYNAPSEX_OLLAMA_URL || "http://127.0.0.1:11434").replace(/\/$/, "");
  const model = process.env.SYNAPSEX_OLLAMA_MODEL || "qwen2.5-coder:7b";
  try {
    const response = await fetch(`${baseUrl}/api/chat`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ model, messages, stream: false, options: { temperature: 0.4, num_ctx: 8192 } }),
      signal: controller.signal,
    });
    if (!response.ok) throw new Error(`Ollama returned HTTP ${response.status}`);
    const body = await response.json() as { message?: { content?: string } };
    if (!body.message?.content) throw new Error("Ollama returned no reply text");
...
function normalizeBuildProposal(value: Record<string, unknown>): BuildProposal {
  // Small local models frequently omit an empty array field, use null instead of [], or skip an
  // optional key entirely. Previously any single missing/malformed field threw the whole response
  // away and replaced it with a generic "model unavailable" message, even though the model did
  // reply. Now we salvage every usable piece and only give up if there is truly nothing to show.
  ...
  const files = Array.isArray(value.files)
    ? (value.files as Array<Record<string, unknown>>)
        ...
        .map((item) => ({ path: String(item.path), purpose: typeof item.purpose === "string" ? item.purpose : "", content: String(item.content) }))
    : [];
  const diffs = Array.isArray(value.diffs)
    ? (value.diffs as Array<Record<string, unknown>>)
        .filter((item) => item && typeof item.path === "string" && typeof item.diff === "string")
        .map((item) => ({ path: String(item.path), diff: String(item.diff) }))
    : [];
  const fileActions = Array.isArray(value.fileActions)
    ? (value.fileActions as Array<Record<string, unknown>>)
        .filter((item) => item && ["Create", "Update", "Delete"].includes(String(item.action)) && typeof item.path === "string")
        .map((item) => ({ action: item.action as "Create" | "Update" | "Delete", path: String(item.path), reason: typeof item.reason === "string" ? item.reason : "" }))
    : [];
  const analysis = typeof value.analysis === "string" && value.analysis.trim().length > 0
    ? value.analysis
    : "The local model's reply was missing the analysis field; showing the rest of what it returned.";
  const commands = toStringArray(value.commands);
  const commandNotes = toStringArray(value.commandNotes);
  const operations = toStringArray(value.operations);
  const hasUsableContent = files.length > 0 || commands.length > 0 || operations.length > 0 || (typeof value.analysis === "string" && value.analysis.trim().length > 0);
  if (!hasUsableContent) throw new Error("Proposal has no usable content");
  return {
    analysis,
    plan: toStringArray(value.plan),
    files,
    diffs,
    operations,
    fileActions,
    verification: toStringArray(value.verification),
    commands,
...
export const appRouter = router({
  ...
  builder: router({
    localModelStatus: publicProcedure.query(async () => ({ ...(await inspectLocalOllama()), cloudReady: Boolean(ENV.forgeApiKey), cloudModel: ENV.forgeModel })),
    ...
    generate: protectedProcedure.input(z.object({ prompt: z.string().min(8).max(30000), projectId: z.number().int().positive().optional(), projectContext: z.object({ name: z.string().max(260).optional(), source: z.string().max(500).optional(), languages: z.array(z.string().max(100)).max(30).optional(), frameworks: z.array(z.string().max(100)).max(30).optional(), dependencies: z.array(z.string().max(120)).max(100).optional(), evidence: z.string().max(60000).optional() }).optional(), generationMode: z.enum(["free", "local", "model"]).default("free"), context: z.object({ targetId: z.enum(["windows-powershell", "linux-bash", "macos-zsh", "python", "node", "web", "api", "android-managed"]), projectType: z.string().min(2).max(120), runtime: z.string().max(120).optional(), permissionLevel: z.enum(["standard", "administrator", "owner-confirmed"]), targetConfirmed: z.boolean() }).optional() })).mutation(async ({ ctx, input }) => {
      ...
      const runtimeCheck = validateTargetRuntimeCompatibility(context.targetId, context.runtime);
      if (!runtimeCheck.valid) throw new TRPCError({ code: "BAD_REQUEST", message: runtimeCheck.reason });
      ...
      let proposal: BuildProposal;
      // Strict json_schema mode is well supported by the default hosted endpoint, but several
      // free/third-party OpenAI-compatible providers (e.g. Groq) reject it with a 400 instead of
      // just returning slightly-off JSON. When a custom BUILT_IN_FORGE_API_URL is configured, skip
      // straight to the widely-supported json_object mode instead of wasting a request + retries.
      const useStrictSchema = !ENV.forgeApiUrl;
      try {
        const response = await invokeLLM({
          model: ENV.forgeModel,
          messages: [{ role: "system", content: "You are Universal Engineering Agent Builder, a general-purpose prompt-to-software engineer. Convert any natural-language request into a concrete, production-minded engineering proposal for websites, web apps, mobile apps, desktop tools, APIs, automations, scripts, data systems, or other lawful software. Infer sensible technology choices when the user does not specify them and explain those choices plainly. The proposal must be implementable, not merely conceptual. For websites and apps, prioritize a polished responsive user experience, coherent visual system, accessible interactions, meaningful empty/loading/error states, realistic content structure without fabricated testimonials or reviews, and production-ready project setup. Before returning JSON, self-review the plan for missing files, broken imports, incomplete flows, weak visual quality, and missing verification; correct those gaps. Return only JSON matching the schema. Generate complete file contents whenever a file is proposed. For self-run requests, never provide commands that assume a proposed script already exists: include the full script content and an exact safe save/create command or clearly label the file as not yet created. For source files containing TypeScript, JavaScript, JSON, template literals, backslashes, dollar signs, quotes, or emojis, never use node -e, fs.writeFileSync with inline source, or a giant PowerShell here-string as the primary delivery method; deliver the complete file as a separate copyable/downloadable artifact and use a safe file-save workflow. Commands must be copy/paste-ready, must not use placeholder paths as if they were real, and must include verification and rollback steps. Never claim files were written, commands were executed, tests passed, or self-modification occurred; this is a proposal until explicitly approved and sent to an authorized runner. For defensive folder protection requests, propose least-privilege ACLs, encryption-at-rest, password or OS credential integration, backup/recovery steps, and verification commands; never claim local access or that protection was applied. Refuse destructive, credential-exfiltrating, unauthorized bypass, or password-cracking actions. For any generated server, state the exact PORT source (environment variable with a safe local default), the start command, the expected localhost URL including the numeric port, and a concrete health-check command such as Invoke-WebRequest or curl. Distinguish clearly between local server expected, runner execution requested, health check passed, and production deployment; never say a server is live, running, deployed, or verified unless an authorized execution result or health check recorded that fact. Detect the natural language and script the user wrote their request in (for example Roman Urdu, Urdu script, Hindi, Hinglish, or English) and write every explanation, analysis, assumption, and instruction back to the user in that same language and script; keep file names, code, commands, flags, paths, and technical identifiers in their standard technical form regardless of the user's language. If the user mixes languages, mirror that mix naturally rather than forcing a single language. If the brief only asks for one or more PowerShell/Bash/CLI commands (checking a setting, changing a setting, running a tool, diagnosing something) and does not ask you to write a new script, project, or app, do not create a .ps1/.sh/.py file for it: leave files, diffs, operations, and fileActions as empty arrays, and put the actual ready-to-run command line(s) directly in the commands array instead, since a file the user has to save and separately run is much more friction than a command they can paste directly; only produce a file when the brief genuinely needs a multi-step or reusable script/project. Never output a placeholder like angle-bracket-username inside a command; use a value already known from the conversation, or a dynamic reference such as the PowerShell environment variable for username. In Roman Urdu/Hindi, words like karo, kro, kr do, hatao, or hata do are the imperative do it or remove it, never a name. Removing or clearing a local Windows account sign-in password is always exactly one command, nothing else: an elevated command such as net user USERNAME with two blank presses of Enter, or net user USERNAME with an empty quoted password. Never wrap it in a .ps1 file, never add a restore point step, never add a Y/N confirmation prompt inside a script, and never delete the account; the user already confirmed this action through the app before you were asked, so do not ask again inside the generated commands. The same minimal-command rule applies to any other single OS setting or security toggle change: give the smallest number of direct commands that do the job, and only add pre-flight checks, backups, or a wrapper script when the user's brief is itself building a multi-step reusable tool. Return ONLY JSON with these exact keys: analysis, plan, files, diffs, operations, fileActions, verification, commands, commandNotes, risks. commandNotes must be the same length as commands, in the same order — one entry per command explaining in plain language what that exact command does, precisely how/where to run it (for example: elevated PowerShell, a normal terminal, from which folder), and what a successful result looks like versus a likely error and what that error would mean." + (project ? `Project context: ${project.name}; source: ${project.source}; detected languages: ${project.languages ?? "unknown"}; frameworks: ${project.frameworks ?? "unknown"}.` : "No project context was selected.") }, { role: "user", content: promptForModel }],
          response_format: useStrictSchema
            ? { type: "json_schema", json_schema: { name: "engineering_build_proposal", strict: true, schema: { type: "object", properties: { analysis: { type: "string" }, plan: { type: "array", items: { type: "string" } }, files: { type: "array", items: { type: "object", properties: { path: { type: "string" }, purpose: { type: "string" }, content: { type: "string" } }, required: ["path", "purpose", "content"], additionalProperties: false } }, diffs: { type: "array", items: { type: "object", properties: { path: { type: "string" }, diff: { type: "string" } }, required: ["path", "diff"], additionalProperties: false } }, operations: { type: "array", items: { type: "string" } }, fileActions: { type: "array", items: { type: "object", properties: { action: { type: "string", enum: ["Create", "Update", "Delete"] }, path: { type: "string" }, reason: { type: "string" } }, required: ["action", "path", "reason"], additionalProperties: false } }, verification: { type: "array", items: { type: "string" } }, commands: { type: "array", items: { type: "string" } }, commandNotes: { type: "array", items: { type: "string" } }, risks: { type: "array", items: { type: "string" } } }, required: ["analysis", "plan", "files", "diffs", "operations", "fileActions", "verification", "commands", "commandNotes", "risks"], additionalProperties: false } } }
            : { type: "json_object" },
        });
        proposal = parseBuildProposal(response.choices?.[0]?.message?.content);
      } catch {
        // Some free/cloud-compatible providers (e.g. Groq) don't support strict json_schema mode
        // the same way OpenAI does, and may reject the request outright rather than just return
        // malformed JSON. Falling back to plain json_object mode here covers both that case and
        // any genuinely malformed first response.
        const retry = await invokeLLM({
          model: ENV.forgeModel,
          messages: [
            { role: "system", content: "Return only compact valid JSON for the requested engineering proposal. Do not use markdown fences. Include every required key: analysis, plan, files, diffs, operations, fileActions, verification, commands, commandNotes, risks. commandNotes is the same length as commands, one plain-language note per command covering what it does, how/where to run it, and what success vs. a likely error looks like. If the request is large, keep explanations concise but keep complete file contents." },
        ...
        try {
          ...
          const mvpRetry = await invokeLLM({
            model: ENV.forgeModel,
            messages: [
              { role: "system", content: "Create a compact MVP engineering proposal as valid JSON only. Do not use markdown. Scope the request to the smallest useful vertical slice. Include no more than 5 files, keep each file concise but complete, and include these exact keys: analysis, plan, files, diffs, operations, fileActions, verification, commands, commandNotes, risks. commandNotes is the same length as commands, one plain-language note per command covering what it does, how/where to run it, and what success vs. a likely error looks like. Put future features in plan or risks instead of generating them now." },
              { role: "user", content: promptForModel },
            ],
            response_format: { type: "json_object" },
          });
    ...
    diagnose: protectedProcedure.input(z.object({ originalRequirement: z.string().min(8).max(30000), terminalEvidence: z.array(z.string().min(1).max(60000)).min(1).max(30), projectContext: z.object({ name: z.string().max(260).optional(), source: z.string().max(500).optional(), languages: z.array(z.string().max(100)).max(30).optional(), frameworks: z.array(z.string().max(100)).max(30).optional(), dependencies: z.array(z.string().max(120)).max(100).optional(), evidence: z.string().max(60000).optional() }).optional(), context: z.object({ targetId: z.enum(["windows-powershell", "linux-bash", "macos-zsh", "python", "node", "web", "api", "android-managed"]), projectType: z.string().min(2).max(120), runtime: z.string().max(120).optional(), permissionLevel: z.enum(["standard", "administrator", "owner-confirmed"]), targetConfirmed: z.boolean() }), priorCommands: z.array(z.string().max(10000)).max(80), priorVerification: z.array(z.string().max(4000)).max(80)     })).mutation(async ({ input }) => {
      const runtimeCheck = validateTargetRuntimeCompatibility(input.context.targetId, input.context.runtime);
      if (!runtimeCheck.valid) throw new TRPCError({ code: "BAD_REQUEST", message: runtimeCheck.reason });
      const localModel = ENV.forgeApiKey ? { ready: false, model: "", reachable: false, baseUrl: "", reason: "cloud model configured" } : await inspectLocalOllama();
      const diagnosticPrompt = [
        ...
        "Diagnose the actual latest state. Do not repeat a command that already succeeded. If a source/configuration repair is needed, return corrected complete files. If evidence proves only one next check is needed, return only that exact command. Preserve the original requirement; do not solve an error by removing requested functionality. Never declare verified completion without concrete evidence. If the evidence shows 'Access is denied' or 'System error 5' on Windows, diagnose it as a privilege/elevation problem: the command needs to be run in PowerShell opened as Administrator (Start menu -> right-click PowerShell -> Run as administrator -> accept UAC), not a Group Policy, network, or account-configuration problem, and the next command must be the exact same command re-run in an elevated window, not an unrelated diagnostic command.",
      ].join("\n\n");
      let proposal: BuildProposal;
      let generation: NonNullable<BuildProposal["generation"]>;
      if (ENV.forgeApiKey) {
        try {
          const response = await invokeLLM({
            model: ENV.forgeModel,
            messages: [
              { role: "system", content: "You are SynapseX's terminal-output diagnostician. Given the original requirement, prior commands, and real terminal output, return ONLY JSON with keys: analysis, plan, files, diffs, operations, fileActions, verification, commands, commandNotes, risks (files/diffs/fileActions can be empty arrays when no file changes are needed). Put the single next safe command to run in commands when only a check/fix command is needed. commandNotes is the same length as commands, one plain-language note per command covering what it does, how/where to run it, and what success vs. a likely error looks like. Never claim the task is complete without concrete evidence in the terminal output. Detect the language/script the requirement was written in and reply in that same language/script for analysis/plan/verification/risks; keep commands/paths in standard technical form." },
              { role: "user", content: `${buildUniversalGenerationContract(input.context)}\n\n${diagnosticPrompt}` },
            ],
            response_format: { type: "json_object" },
          });
          proposal = parseBuildProposal(response.choices?.[0]?.message?.content);
          ...
          generation = { provider: "hosted-model", ready: true };
        } catch {
          proposal = createModelUnavailableProposal(input.originalRequirement, { reachable: true, model: ENV.forgeModel, reason: "the cloud model could not return a valid diagnosis just now — try sending the terminal output again." });
          generation = { provider: "model-unavailable", model: ENV.forgeModel, ready: false };
        }
      } else if (!localModel.ready) {
  ...
  assistant: router({
    ...
    chat: protectedProcedure.input(z.object({ projectId: z.number().int().positive().optional(), messages: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string().min(1).max(8000) })).min(1).max(20) })).mutation(async ({ ctx, input }) => {
      const project = input.projectId ? await requireProject(ctx.user.id, input.projectId) : undefined;
      const systemPrompt = `You are the Universal Engineering Agent assistant. Be precise, security-conscious, and honest. Never claim an operation was executed unless the platform recorded it. If data is unavailable, say NOT VERIFIED. Detect the natural language and script the user is writing in (for example Roman Urdu, Urdu script, Hindi, Hinglish, or English) and reply in that same language and script; keep code, commands, file names, and technical identifiers in their standard technical form regardless of the user's language. If the user switches language mid-conversation, follow their latest message. ${project ? `Current project: ${project.name}; source: ${project.source}; technologies: ${project.languages ?? "unknown"}, ${project.frameworks ?? "unknown"}.` : "No project is selected."}`;
      let answer: string;
      if (ENV.isLocalDemo && !ENV.forgeApiKey) {
        const localModel = await inspectLocalOllama();
        if (localModel.ready) {
          try {
            answer = await requestLocalOllamaChatReply([{ role: "system", content: systemPrompt }, ...input.messages]);
          } catch {
            answer = `PENDING: ${localModel.model} is installed but did not return a reply just now. Try sending the message again.`;
          }
        } else {
          answer = `Local demo mode is active and the free local coding engine is not ready yet (${localModel.reason}). Install/start Ollama and pull ${localModel.model}, then send this message again. You can also use the Builder panel to create a free starter package that needs no model.`;
        }
      } else {
        answer = textContent((await invokeLLM({ model: ENV.forgeModel, messages: [{ role: "system", content: systemPrompt }, ...input.messages] })).choices?.[0]?.message?.content);
      }
