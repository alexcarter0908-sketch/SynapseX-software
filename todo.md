...
- [x] Assess the current product against the intended multilingual, platform-aware command and defensive-security guidance purpose; define missing capabilities before any implementation.
- [x] Reposition SynapseX as a universal self-run prompt-to-software studio rather than a runner-dependent engineering workspace.
- [x] Add explicit platform, shell/runtime, project type, permission, and execution-mode inputs that turn free-text prompts into a safe generation contract.
- [x] Generate complete downloadable artifacts with exact install, preflight, run, verification, and rollback steps for supported targets without requiring a runner.
- [x] Establish a free-first local-model and deterministic-template architecture without making a paid model provider mandatory.
- [x] Add safe defensive-security request classification, owner/authorization confirmation, command validation, and critical-action safeguards.
- [x] Implement the first universal self-run foundation: target-aware generation context, free-first preflight artifact package, platform-specific starter commands, self-run default, and owner-confirmed critical-action guardrails.
- [x] Make the SynapseX full integration-test suite use an isolated migrated test database or deterministic database mocks; the cloned project currently points at a database missing its own `activity` table and must not use the CRM database for tests.
- [x] Add a visible runtime/framework selector so users can explicitly request PowerShell, Bash, Python, Node.js, FastAPI, or another supported stack alongside the selected target platform.
- [x] Add deterministic self-run starter artifacts for common website, FastAPI API, PowerShell automation, and authorized defensive-security baseline requests before handing off to a local model for larger custom codebases.
- [x] Reunify the Assistant into one clean PowerShell/Visual Studio-style command surface with the chat, scrollbar, artifacts, and composer inside the same readable panel.
- [x] Remove unnecessary visual separation, compact oversized controls, and widen/tall the Assistant workspace while preserving all current safety, history, and self-run behavior.
- [x] Add deterministic self-run starter artifacts for Node.js services, React applications, and local database applications with install, run, verification, and rollback guidance.
- [x] Remove the horizontal overflow in the unified Assistant at narrow desktop widths while keeping the chat, Builder controls, and composer readable in the same workspace.
- [x] Add a self-contained Windows local-demo mode that avoids invalid external OAuth URLs and renders the Assistant safely without production authentication or CRM data. The explicit non-production mode uses a non-persistent Local Demo identity, does not open a database handle for Builder/chat, supports free/local self-run packages, and is covered by 48 tests, TypeScript, production build, and a localhost browser check.
- [x] Replace the multi-panel Assistant interface with one large prompt editor, direct response/artifact view, and a single History control.
- [x] Remove prompt-time Builder selectors, runner controls, approval counters, and other unnecessary configuration from the normal local self-run workflow.
- [x] Add an explicit self-improvement proposal flow that can inspect the SynapseX workspace and generate complete reviewed changes, tests, verification, and rollback instructions without claiming automatic execution.
- [x] Expand target-aware generation and defensive-security guidance for lawful, authorized software, infrastructure, hardware-design, and automation requests while preserving refusal of unsafe or unauthorized actions.
- [x] Create a representative quality-validation matrix for prompts, platform commands, generated artifacts, error handling, local-demo behavior, history, and responsive UI.
- [x] Package a new Windows ZIP only after automated checks, browser validation, and the representative quality matrix pass. Archive integrity passed; the ZIP contains the local-demo example, simplified prompt studio source/tests, and quality validation record while excluding node_modules and real environment files.
- [x] Fix the prompt studio response pane so generated content uses a readable full-page workspace with normal page scrolling, not a short nested scroll area below the prompt. After generation, the prompt editor compacts, the response scrolls into view, and all commands/files/verification remain readable with page-level scrolling.
- [x] Fix clear authorized PowerShell backup prompts so they return a parameterized dated-ZIP backup script and exact commands, instead of the generic Start-AuthorizedTask starter or an incorrect critical-risk label. The exact reported prompt now returns New-DatedProjectBackup.ps1, source/backup paths, dated ZIP verification, manual rollback guidance, and low-risk classification.
- [x] Remove hosted analytics placeholders from the local Windows runtime when analytics variables are absent, preventing malformed `%VITE_ANALYTICS_ENDPOINT%` requests and Express URI errors. Analytics now loads only from a valid configured endpoint and website identifier; clean local startup, browser console, 54 tests, TypeScript, and production build passed.
- [x] Replace JSON-only response downloads with a real ZIP package that preserves safe generated file paths and includes all displayed artifacts, plus a readable response manifest. Browser download and archive validation confirmed the generated ZIP contains the PowerShell script, rollback document, and SYNAPSEX-MANIFEST.json.
- [x] Correct backup-path extraction so natural-language Windows backup prompts retain distinct source and destination paths even when English phrasing appears between the two paths. Browser and regression tests confirmed distinct C:\Test\Source and C:\Test\Backups values.
- [x] Fix prompt-studio horizontal overflow that cuts off the Generate and Download ZIP controls, ensuring the response action remains visible at the user's desktop width. Page/root width now stays within the viewport, prompt and response controls stack responsively, and the Download ZIP action remained visible in browser validation.
- [x] Replace read-only defensive Windows protection-layer guidance with an authorized executable implementation package that has explicit reversible changes, verification, and rollback. The generated ZIP now includes a Firewall/Defender installer, read-only verifier, controls config, timestamped state record, and report-only cleanup script that never weakens protections.
- [x] Handle Microsoft Defender service-unavailable or third-party-security scenarios without a raw Set-MpPreference error, while clearly reporting that Defender could not be enabled and preserving completed firewall enforcement. Generated installer and verifier now detect WinDefend service availability, record clear warnings, and do not attempt unsupported Defender preference changes.
- [x] Add an authorized Windows sign-in protection package that enforces password requirement after wake, verifies account sign-in protection, never stores or exposes a password, and provides a safe local rollback option. Generated ZIP includes the wake-sign-in installer, interactive local password script, verifier, and explicit wake-setting rollback; browser ZIP inspection, 59 tests, TypeScript, and production build passed.
- [x] Fix Roman Urdu/Hindi/English mixed intent routing so “sleep se wake par password” and equivalent prompts produce the Windows sign-in implementation package instead of the generic PowerShell starter. Focused and full routing tests passed.
- [x] Add a universal custom-generation fallback that clearly routes unfamiliar authorized development requests to a local open-source model or optional configured model rather than pretending a generic starter is a complete implementation. Focused tests, full suite, TypeScript, and production build passed.
- [x] Add automatic Windows ZIP extraction, package-location, preview, apply, verification, and rollback guidance to every generated response so users do not need manual folder discovery or repeated chat assistance. Focused tests, full suite, TypeScript, and production build passed.
- [x] Include self-guided run and verification documentation inside every downloaded generated ZIP, with safe PowerShell commands that verify expected scripts before execution. Each Windows package now includes RUN-ME-FIRST.ps1 and docs/WINDOWS-ZIP-RUN-GUIDE.md.
- [x] Add explicit confirmation handling for security-reducing local password-removal requests; do not provide password-removal commands without clear owner/local-account confirmation. Regression coverage passed.
- [x] Detect Roman Urdu password-removal phrases such as “mery pc sy pswrd remove kro” and stop generation with a clear local-account ownership confirmation request instead of routing to Ollama. Shared and server routing regression coverage passed.
- [x] Add generated-package guidance that detects and rejects an old/existing extraction folder before users run a new package from the wrong directory. Local-model packages now verify their required files in RUN-ME-FIRST.ps1 and explicitly reject older sign-in, backup, or protection folders.
- [x] Add a guarded self-guided local Windows account password-removal package only after an explicit ownership confirmation, with preview, typed final confirmation, verification, and no Microsoft/work-account support. Focused tests, full suite, TypeScript, and production build passed.
- [x] Fix generated local-account password-removal PowerShell scripts so every param block appears before executable statements, preventing the reported `param is not recognized` error. Focused tests, full suite, TypeScript, and production build passed.
- [x] Add a reusable self-guided diagnostic workflow that tells users what output to collect, identifies common package/script/account/power-setting failure categories, and provides safe evidence-based next steps. The diagnostic profile now turns known pasted errors into a diagnosis and read-only evidence plan.
- [x] Include package-local troubleshooting instructions for PowerShell syntax errors, wrong-folder execution, local-versus-Microsoft/work account boundaries, failed policy changes, and unavailable previous-state records. Every generated Windows free package now includes Collect-SynapseXDiagnostics.ps1 and TROUBLESHOOTING.md.
- [x] Add regression coverage for the observed Windows sign-in and password-removal error patterns so future package changes do not reintroduce them. Focused regressions, 64 full tests, TypeScript, and production build passed.
- [x] Fix the false-positive unsafe classification that treats benign phrases such as “root cause” as device rooting, while retaining blocks for actual rooting, jailbreaking, bypassing, credential theft, and harmful-security requests. Focused regressions, 64 full tests, TypeScript, and production build passed.
- [x] Make normal implementation prompts return direct complete source files, commands, expected output, verification, and rollback in the visible response; retain Download ZIP only as an optional export. Focused tests, full suite, TypeScript, and production build passed.
- [x] Keep diagnostics restricted to pasted errors and preserve self-guided ZIP instructions only when the user explicitly downloads an artifact. Direct response commands no longer include ZIP extraction, while the optional ZIP retains its self-guided run documents.
- [x] Add a Yes/No confirmation dialog for high-impact security-reducing requests, including local password removal and wake-sign-in protection reduction; generation does not continue unless Yes is selected. Focused tests, 66 full tests, TypeScript, and production build passed.
- [x] Explain every generated command in plain language, including purpose, expected result, safety effect, and next step. The response now displays these details beside every command.
- [x] Add a separate Copy button for each command while retaining an optional Copy all commands control. Focused command-workflow UI logic tests passed.
- [x] Add a pasted terminal-output workflow that classifies success, failure, and incomplete evidence; provides a root-cause explanation and corrected safe next command where supported. It currently recognizes the observed PowerShell param error, pnpm EPERM, nested PowerShell access denial, missing commands, and generic error signals.
- [x] Report whether the task is complete, needs verification, or has remaining improvements after output analysis. Focused tests, 69 full tests, TypeScript, and production build passed.
- [x] Fix false-positive high-impact confirmation when a normal prompt includes unrelated phrases such as “login nahi” and “existing file delete na ho”; only explicit password-removal or wake-protection-reduction intent may trigger confirmation. Focused tests, 69 full tests, TypeScript, and production build passed.
- [x] Remove irrelevant Node/npm checks and preflight-package wording from direct Python/FastAPI implementation responses; show only runnable Python project files and commands. Focused tests, 69 full tests, TypeScript, and production build passed.
- [x] Detect “Python was not found” terminal output as an explicit runtime-missing error, explain the cause, and provide safe setup/retry guidance instead of an inconclusive status. Focused tests, 69 full tests, TypeScript, and production build passed.
- [x] Audit and replace the package-first fallback so simple authorized operating-system requests such as “open notepad” produce a direct native command rather than an unnecessary ZIP or local-model route. Native Notepad, Calculator, Paint, and File Explorer actions now use a reviewed one-command path.
- [x] Add a structured execution-tier router: direct native command, deterministic project implementation, optional local-model generation only for unsupported custom work, and explicit confirmation for high-impact security reductions.
- [x] Replace generic command labels with task-specific purpose, expected evidence, safety boundary, verification, rollback, and completion criteria. The studio shows the engineering workflow and each command’s explanation beside its copy control.
- [x] Expand terminal-output analysis into prerequisite detection, error taxonomy, safe corrected next steps, and a clear complete / needs-verification / remaining-work outcome. It covers Python/winget availability, dependency/import failures, local port conflicts, Windows application launch errors, pnpm EPERM, nested PowerShell denial, and known PowerShell syntax failures.
- [x] Add end-to-end regression coverage across direct Windows commands, FastAPI implementations, local-model fallback, confirmation gates, and observed error recovery paths. Focused regressions, 71 full tests, TypeScript, and production build passed.
- [x] Analyze terminal output chronologically so a later successful prerequisite installation resolves an earlier missing-runtime error instead of repeating stale repair commands. It now distinguishes installer metadata from a real post-install `python --version` response.
- [x] Maintain a per-response execution state that reports the current next command, confirmed completed steps, and remaining evidence rather than re-evaluating every pasted output as a first attempt. The output panel preserves each pasted terminal step in the active implementation session.
- [x] Add a dedicated implementation workspace chooser and browser-based direct file-save flow so users can save shown files before running commands without downloading a ZIP. Chrome/Edge users select a folder and save the exact visible files directly through the browser.
- [x] Prevent run guidance from treating an unrelated earlier generated-package folder as the implementation project workspace. Project command copy controls stay disabled until visible files are saved into the selected workspace. Focused tests, 72 full tests, TypeScript, and production build passed.
- [x] Treat Windows Package Manager outcomes stating that Python is already installed or has no available upgrade as prerequisite resolution, then require a new PowerShell `python --version` check instead of repeating stale install guidance. Focused tests, 73 full tests, TypeScript, and production build passed.
- [x] Define and implement a consolidated engineering-assistant acceptance matrix covering prompt interpretation, direct command routing, deterministic implementations, workspace save, command sequencing, runtime prerequisites, multi-step output memory, repair guidance, verification, completion reporting, history, and guarded changes. See docs/ENGINEERING_ASSISTANT_ACCEPTANCE.md.
- [x] Run all acceptance scenarios as one release gate and publish a single capability-boundary guide that distinguishes deterministic support from optional local-model custom generation. Focused acceptance regressions, 75 full tests, TypeScript, and production build passed.
- [x] Replace generic keyword routing that classified the submitted Next.js website brief as a FastAPI API and produced an unrelated Python starter. Detailed website/framework intent now takes precedence over embedded product-stack mentions.
- [x] Parse detailed implementation briefs into a request-aware project specification that selects the correct architecture, files, dependencies, commands, verification, and implementation tier. Native commands, deterministic project profiles, guarded changes, and local-model work use separate routes.
- [x] Add a real deterministic Next.js/Tailwind/Framer Motion multi-page website profile for the submitted SynapseX Ocean Deep ecosystem showcase brief. The profile includes App Router pages, five factual product routes, shared components, responsive Ocean Deep styles, neural animation, reduced-motion behavior, and local build commands.
- [x] Add acceptance tests proving detailed website prompts no longer collapse into API starters while FastAPI and native-command prompts retain their correct routes. Focused routing regressions passed.
- [x] Integrate a user-controlled free local Ollama generation path that converts rich arbitrary briefs into structured project files instead of returning a generic starter; preserve the same workspace, command, output-analysis, repair, and verification loop. It uses a live local Ollama endpoint when available and falls back to transparent local setup guidance otherwise.
- [x] Use the submitted SynapseX Ocean Deep website brief as an end-to-end rich-project acceptance fixture, including its requested framework, pages, animation requirements, and factual product content boundaries. Full validation passed: 75 tests, TypeScript, and production build.
- [x] Replace generic command labels in direct implementation responses with command-specific purpose, expected evidence, safety context, and immediate next action for Node, npm, Python, virtual-environment, server, and verification commands. Focused command/routing tests plus the full test suite, TypeScript check, and production build passed.
- [x] Audit every deterministic profile and fallback against the submitted requirement for a genuine general-purpose, task-stateful, model-backed engineering assistant; remove any behavior that represents a template or scaffold as arbitrary task implementation. The audit is documented, non-trivial requests now use local Ollama, model failure is explicitly pending, and the staged scaffold fallback was removed.
- [x] Introduce one universal task-state contract that preserves the original requirement, project context, implementation plan, changed files, generated commands, terminal timeline, diagnostics, verification evidence, pending external prerequisites, and final task status across turns. Shared-contract tests cover pending work, chronology, repairs, and evidence-gated completion.
- [x] Add universal model-backed prompt interpretation, implementation, error-repair, and completion reporting, while keeping deterministic helpers only as verified primitives and never as disguised arbitrary-code generation. Local-model generate and diagnose endpoints now preserve the original requirement and project context through the repair loop.
- [x] Add project inspection intake and a broad acceptance matrix for new projects, existing-project modifications, successful output, failing output, multi-step work, ambiguity, credentials, and completion gates. Browser project-context inspection, universal task-state and diagnosis tests, the rewritten acceptance matrix, local browser smoke test, full test suite, TypeScript check, and production build passed.
- [x] Add task-specific command guidance for the Ollama installation, model-download, model-list, and model-version commands used by the truthful pending-model state. Focused command-guidance regression tests passed.
