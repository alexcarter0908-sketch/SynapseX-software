...
export type TargetRuntimeValidation =
  | { valid: true }
  | { valid: false; reason: string };

export function validateTargetRuntimeCompatibility(targetId: UniversalTargetId, runtime?: string): TargetRuntimeValidation {
  const value = runtime?.trim().toLowerCase();
  if (!value) return { valid: true };
  const target = getUniversalTarget(targetId);
  const has = (pattern: RegExp) => pattern.test(value);

  if (targetId === "windows-powershell" && has(/\b(?:bash|zsh|sh|unix|linux|macos)\b/)) {
    return { valid: false, reason: `${target.label} requires a PowerShell-compatible runtime; the supplied runtime names a Unix shell or platform.` };
  }
  if ((targetId === "linux-bash" || targetId === "macos-zsh") && has(/\b(?:powershell|pwsh|cmd(?:\.exe)?|windows)\b/)) {
    return { valid: false, reason: `${target.label} requires its Unix shell; the supplied runtime names Windows PowerShell or CMD.` };
  }
  if (targetId === "python" && has(/\b(?:node(?:\.js)?|npm|deno)\b/)) {
    return { valid: false, reason: "Python target cannot use a Node.js runtime override. Choose Python or switch the target to Node.js." };
  }
  if (targetId === "node" && has(/\b(?:python|pip|uv)\b/)) {
    return { valid: false, reason: "Node.js target cannot use a Python runtime override. Choose Node.js or switch the target to Python." };
  }
  return { valid: true };
}
