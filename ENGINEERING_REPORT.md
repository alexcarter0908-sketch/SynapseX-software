...
## UPDATE — EXPLICIT TARGET CONTRACT

The Prompt-to-Code Studio now offers an explicit target environment selector with a recommendation mode that preserves the previous prompt inference behavior. Supported targets come from the shared universal contract: Windows PowerShell, Linux Bash, macOS Zsh, Python, Node.js, website/web application, API/backend, and authorized Android managed device. Users may also provide an optional runtime or version override; blank input uses the selected target default. The selected target and runtime are sent through the existing `builder.generate` context, saved with local response history, restored when history is reopened, and reused during model diagnosis. Existing approval, risk, self-run, runner, complete-file, verification, rollback, and truthful execution-status boundaries are unchanged.

Focused regression coverage now proves recommendation mode remains compatible and an explicit target overrides conflicting prompt wording. The full available repository gate passed: 15 test files and 80 tests, TypeScript check, and production build. The build still emits a large-chunk warning from the existing visualization bundle; it is non-blocking and unrelated to this change. Desktop and 390px browser verification remains a follow-up for the isolated ZIP because it is not the active managed webdev project in this session.
